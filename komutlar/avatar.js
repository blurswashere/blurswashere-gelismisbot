...
// Diyelimki bir kullanıcının avatarını çok beğendiniz veya net bir şekilde görmek istiyorsunuz. <prefix>avatar @kullanıcı yazdığınızda o kullanıcının avatarını sizlere gösterir.
...
.setImage(user.displayAvatarURL({dynamic: true, size: 1024}))
.setColor("BLUE") // Embed Mesajı Rengi. İstediğiniz gibi ayarlayabilirsiniz.
message.channel.send(embed)

}
}catch{
  message.channel.send(new Discord.MessageEmbed().setColor("BLUE").setDescription("Belirttiğiniz Kullanıcıyı Bulamadım!"));
  return;
}
